...
class RetrofitCall {
    ...
    private fun fetchParkingData(page: Int, perPage: Int) {
        apiService.getParkingData(serviceKey, page, perPage).enqueue(object : Callback<ParkingDataResponse> {
            override fun onResponse(call: Call<ParkingDataResponse>, response: Response<ParkingDataResponse>) {
                if (response.isSuccessful) {
                    ...
                    for (data in parkingDataList) {
                        Log.d("ParkingData", "주차장 이름: ${data.parkingName}, 주소: ${data.address}")
                    }
                } else {
